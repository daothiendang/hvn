	...
	class WordpressPopularPosts extends WP_Widget {
		...
		public function widget( $args, $instance ) {
			...
			echo $before_widget . "\n";
                        ...
                        if($instance['cat'] == -1){ //random category
                            //random category
                            $list_categories = get_categories();
                            foreach($list_categories as $key => $category){
                                // extract uncategorized (id = 1)
                                if($category->term_id == 1) unset($list_categories[$key]);
                            }
                            $rand_key = array_rand($list_categories);
                            $rand_category = $list_categories[$rand_key];
                            $instance['cat'] = $rand_category->term_id;
                            $instance['title'] = $rand_category->name;
                        }
...
//                                        // extract uncategorized (id = 1)
//                                        if($value == 1) unset($category_ids[$key]);
//                                    }
//                                    $rand_key = array_rand($category_ids);
//                                    $cat_ids[0] = $category_ids[$rand_key];
//                                }
				$in = array();
				$out = array();
				$not_in = "";
